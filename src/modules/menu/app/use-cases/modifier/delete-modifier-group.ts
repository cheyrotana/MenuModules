...
export class DeleteModifierGroupUseCase {
  ...
  async execute(input: {
    ...
    try {
      // 2 - Run checks, delete, and publish event in a transaction
      await this.txManager.withTransaction(async (client) => {
        ...
        // 2.2 - Ensure group is not attached to any menu items
        ...
        if (hasAnyAttachments) {
          throw new Error(
            `Cannot delete modifier group "${group.name}" because it is attached to one or more menu items. ` +
              `Detach it from all items before deleting.`
          );
        }

        // 2.3 - Safe to soft delete
        await this.modifierRepo.softDeleteGroup(groupId, tenantId, client);
